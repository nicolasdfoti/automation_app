...
from automation_system.backend.services import schematics as schematics_svc
...
@router.get("/catalog", response_model=SchematicCatalog)
def catalog() -> SchematicCatalog:
    items = schematics_svc.list_catalog()
    return SchematicCatalog(count=len(items), items=items)


@router.post("/generate", response_model=SchematicResult)
def generate(payload: SchematicGenerateRequest) -> SchematicResult:
    record = target_api.get_property(payload.codigo)
    if record is None:
        raise HTTPException(
