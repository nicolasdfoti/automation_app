...
from automation_system.backend.services import target_api
...
def list_catalog() -> list[dict]:
    """Todas las propiedades con su estado de esquematico, en el shape del catalogo.

    Lectura local de la data compartida para el panel de Esquematicos (sin
    importar codigo de target); los campos esquematico_generado / archivo son
    datos escritos por esta misma pieza al generar los PDFs.
    """
    propiedades = db.read_properties()
    items: list[dict] = []
    for _, row in propiedades.iterrows():
        superficie = row["superficie_m2"]
        archivo = row.get("archivo_esquematico")
        items.append({
            "codigo": str(row["codigo"]),
            "direccion": row["direccion"] if not _is_na(row["direccion"]) else None,
            "superficie_m2": None if _is_na(superficie) else float(superficie),
            "esquematico_generado": bool(row["esquematico_generado"]),
            "archivo_esquematico": None if archivo is None or _is_na(archivo) else str(archivo),
        })
    return items
...
def get_schematic_path(codigo: str) -> tuple[str, Path | None]:
    ...
    """
    record = target_api.get_property(codigo)
    if record is None:
