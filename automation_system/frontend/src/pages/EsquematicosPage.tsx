...
import { generateAllSchematics, generateSchematic, getSchematicDownloadUrl, fetchSchematicCatalog } from "../api/schematics";
...
import type { SchematicCatalogItem, SchematicResult } from "../types";
...
export default function EsquematicosPage() {
  const propertiesApi = useApi(fetchSchematicCatalog, []);
  const [items, setItems] = useState<SchematicCatalogItem[] | null>(null);
  const [dialogProperty, setDialogProperty] = useState<SchematicCatalogItem | null>(null);
  ...
  const pendingColumns: DataColumn<SchematicCatalogItem>[] = useMemo(
    () => [
      ...
      { key: "superficie", header: "Superficie (m2)", align: "right", className: "whitespace-nowrap tabular-nums", render: (p) => (p.superficie_m2 != null ? fmtSurface(p.superficie_m2) : "—") },
      {
        key: "estado",
        header: "Estado",
  ...
  const generatedColumns: DataColumn<SchematicCatalogItem>[] = useMemo(
    () => [
      { key: "codigo", header: "Código", className: "whitespace-nowrap font-mono font-medium text-slate-900", render: (p) => p.codigo },
      { key: "direccion", header: "Dirección", className: "max-w-xs truncate text-slate-700", render: (p) => <span className="block max-w-xs truncate">{p.direccion}</span> },
      {
...
interface GenerateDialogProps {
  property: SchematicCatalogItem | null;
  busy: boolean;
  onCancel: () => void;
